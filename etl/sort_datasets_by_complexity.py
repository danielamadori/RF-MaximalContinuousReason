#!/usr/bin/env python3
"""Sort datasets by Random Forest size and number of features.
...
When multiple datasets share the same estimator count, the number of features is
...
def _extract_n_features(metadata: Mapping[str, object] | None) -> int:
    """Return the number of features recorded for a dataset."""
...
def _summarise_datasets(report: Sequence[Mapping[str, object]]) -> List[DatasetComplexity]:
    ...
    for entry in report:
        ...
        n_features = _extract_n_features(
...
def _sort_datasets(
    ...
    """Sort datasets by estimator count and number of features."""
    ...
    def _key(item: DatasetComplexity) -> tuple[int, int, str]:
        ...
        if ascending:
            return (estimators, features, item.dataset)
...
def _print_table(datasets: Sequence[DatasetComplexity]) -> None:
    ...
    header_features = "N Features"

    print(f"{header_dataset}  {header_estimators:>17}  {header_features:>10}")
    print(f"{'-' * name_width}  {'-' * 17:>17}  {'-' * 10:>10}")

    for item in datasets:
        dataset = item.dataset.ljust(name_width)
        estimators = _format_estimators(item.n_estimators).rjust(17)
        features = f"{item.n_features}".rjust(10)
        print(f"{dataset}  {estimators}  {features}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Random Forest and number of features."
        )
    )
    parser.add_argument(
        "--report",
