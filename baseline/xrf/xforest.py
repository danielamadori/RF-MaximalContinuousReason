...
def get_time():
    """Get CPU time, compatible with Windows and Unix."""
...
from baseline.xrf.tree import Forest

from baseline.xrf.encode import SATEncoder, MXEncoder
from baseline.xrf.explain import SATExplainer, MXExplainer
...
class XRF(object):
    ...
    def __init__(self, model, features, classes, verb=0):
        ...
        try:
            self.f = Forest(model.estimators_, self.ffnames)
    ...
    def encode(self, inst, etype='maxsat'):
        ...
        if 'f' not in dir(self):
            try:
                self.f = Forest(self.cls.estimators_, self.ffnames)
            except AttributeError:
        ...
        if self.verbose:
            ...
            print('c encoding time: {0:.3f}'.format(elapsed_time))            
        
    def explain(self, inst, xtype='abd', etype='maxsat', smallest=False, sample_index=None):
        """
        ...
        if self.verbose:
            print("c Total time: {0:.3f}".format(elapsed_time))
