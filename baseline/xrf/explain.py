...
try:
    ...
    # pypblib not available on Windows without build tools
    PBEnc = None
...
class SATExplainer(object):
    ...
    def explain(self, sample, xtype='abd', smallest=False, sample_index=None):
        """
    ...
    def enumerate(self, sample, xtype='con', smallest=False, xnum=10):
        ...
        if self.verbose:
            print('c expl time: {0:.3f}'.format(elapsed_time))
        #
        #self.slv.delete()
        #self.slv = None
        return expls
...
#             print(len(cls.clauses))
            ...
            if PBEnc is None:
                raise NotImplementedError("PBEnc requires pypblib which needs C++ build tools on Windows. "
                                        "Install Visual C++ Build Tools or use CardEnc instead.")
